...
#
# Required one-time host fixes (see sim/README.md "QGroundControl" section):
...
if [[ ! -x "$APPRUN" ]]; then
    ...
    echo "  or override:  QGC_DIR=/path/to/squashfs-root sim/qgc.sh" >&2
    exit 1
fi
...
echo "  MAVLink    : auto-binds UDP 14550 (run mavproxy --out udp:127.0.0.1:14550)"
echo
